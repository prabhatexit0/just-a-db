...
public:
  ...
  [[nodiscard]] auto SelectAll() const -> std::vector<const Tuple *> { return tuples_; }
  auto AddColumn(const Column *column) -> Result<const Column *>;
  auto DropColumn(const std::string& column_name) -> Result<bool>;
...
public:
  ...
  auto CreateTable(const std::string& name, std::vector<const Column *> columns)
      -> Result<const Table *>;
  auto DropTable(const std::string& table_name) -> Result<bool>;
  auto UpdateTable(const std::string& table_name, const Table *table)
      -> Result<const Table *>;
